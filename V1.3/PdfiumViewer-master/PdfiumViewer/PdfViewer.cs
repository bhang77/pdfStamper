...
namespace PdfView
{
    ...
    public partial class PdfViewer : UserControl
    {
        ...
        private int _licstatus;

        public int LicenseStatus
        {
            get { return _licstatus; }
        }
        /// <summary>
        ...
        public PdfViewer()
        {
            ...
            ShowToolbar = true;
            ShowBookmarks = true;
            if (!LIC.clsLicense.IsLicenseFileExist())
            {
                LIC.clsLicense.Generate_NewLicenseFile();
                _licstatus = LIC.clsLicense.LicenseStatus();
            }
            else
            {
               
                _licstatus = LIC.clsLicense.LicenseStatus();
                if(_licstatus==1 || _licstatus==0)
                     LIC.clsLicense.UpdateLastRun();
            }
             UpdateEnabled();
        ...
        private void _saveButton_Click(object sender, EventArgs e)
        {
            using (var form = new SaveFileDialog())
            {
                ...
                if (form.ShowDialog(FindForm()) == DialogResult.OK)
                {
                    try
                    {
                        _renderer.SavePdf(form.FileName);
                    }
                    catch
                    {
                        MessageBox.Show(
        ...
        private void tsbAddImg_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (tsbAddImg.Checked)
                    _renderer.AddInputObject = true;
                else
                    _renderer.AddInputObject = false;
            }
            catch
            { }
        }
