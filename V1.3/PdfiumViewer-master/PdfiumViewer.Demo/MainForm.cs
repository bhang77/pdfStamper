...
namespace PdfiumViewer.Demo
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            ShowPdfLocation(PdfView.PdfPoint.Empty);
        ...
        private void ShowPdfLocation(PdfView.PdfPoint point)
        {
            if (!point.IsValid)
            {
        ...
        private PdfView.PdfDocument OpenDocument(string fileName)
        {
            try
            {
                return PdfView.PdfDocument.Load(this, fileName);
            }
            catch (Exception ex)
            {
        ...
        private void OpenFile()
        {
            ...
            {
                toolStrip1.Enabled = menuStrip1.Enabled= false;

                string msg = "";
                if (pdfViewer1.LicenseStatus == -2)
                {
                    msg = "License file corrupted";
                }
                else if (pdfViewer1.LicenseStatus == -3)
                {
                    msg = "License error : Detection of backdating";
                }
                else if(pdfViewer1.LicenseStatus==-1)
                {
                    msg = "License expired";
                }
        ...
        private void cutMarginsWhenPrintingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            pdfViewer1.DefaultPrintMode = PdfView.PdfPrintMode.CutMargin;
        }
        ...
        private void shrinkToMarginsWhenPrintingToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            pdfViewer1.DefaultPrintMode = PdfView.PdfPrintMode.ShrinkToMargin;
        }
        ...
        {
            FitPage(PdfView.PdfViewerZoomMode.FitWidth);
        }

        private void FitPage(PdfView.PdfViewerZoomMode zoomMode)
        {
        ...
        {
            FitPage(PdfView.PdfViewerZoomMode.FitHeight);
        }

        private void _fitBest_Click(object sender, EventArgs e)
        {
            FitPage(PdfView.PdfViewerZoomMode.FitBest);
        }
        ...
        {
            Rotate(PdfView.PdfRotation.Rotate0);
        }

        private void rotate90ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Rotate(PdfView.PdfRotation.Rotate90);
        }

        private void rotate180ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Rotate(PdfView.PdfRotation.Rotate180);
        }

        private void rotate270ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Rotate(PdfView.PdfRotation.Rotate270);
        }

        private void Rotate(PdfView.PdfRotation rotate)
        {
            // PdfRenderer does not support changes to the loaded document,
            // so we fake it by reloading the document into the renderer.
        ...
        {
            PdfView.PdfInformation info = pdfViewer1.Document.GetInformation();
        ...
        private void mnSave_Click(object sender, EventArgs e)
        {
            try
            {
                using (var form = new SaveFileDialog())
                {
                    ...
                    bool bretval = pdfViewer1.Renderer.SavePdf(form.FileName);
                    if (bretval)
                        MessageBox.Show("Successfully saved the pdf file");
